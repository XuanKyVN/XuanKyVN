...
uint16_t bufferVal[4];

void Convert32_16bit(uint32_t values, uint16_t Short[]){              
        typedef union u_conv
    {
            uint32_t  lValue; //32bit can use uint32_t
            uint16_t lShort[2]; //16bit uint16_t     
    };
    u_conv
    strt,
    finish;
    ...
    for( int i=0; i<2; i++ ){
        Short[i]=strt.lShort[i];
    }
    
  //return Short;
}
//------------------
void Convert16to8bit(uint16_t values, uint8_t byteVal[]){              
        typedef union u_conv
    {
            uint16_t  lValue; //32bit can use uint32_t
            uint8_t lShort[2]; //16bit uint16_t     
    };
    u_conv
    strt,
    finish;
    ...
    for( int i=0; i<2; i++ ){
        byteVal[i]=strt.lShort[i];
    }
}  


//------------------
void ConvertFloatto2x16bit(float values, uint16_t des[]){
typedef union
{
  float v;
  uint16_t b[2];
...
for (int i = 0; i < 2; i++) {
des[i]=floatValue.b[i];
} 
//return des;
}


void Convert64_16bit(uint64_t values, uint16_t Short[]){              
        typedef union u_conv
    {
            uint64_t  lValue; //32bit can use uint32_t
            uint16_t lShort[4]; //16bit uint16_t     
    };
    u_conv
    strt,
    finish;
  ...
  //return Short; // Can not USE uint64 bit for ESP32 and8266
}

uint16_t ConvertInt2Uint16(int16_t valueInt){
  uint16_t valUint=valueInt+pow(2,16);
  return valUint;
}

uint32_t ConvertInt2Uint32(int32_t valueInt){
  uint32_t valUint=valueInt+pow(2,32);
  return valUint;
}
uint64_t ConvertInt2Uint64(int64_t valueInt){
  uint64_t valUint=valueInt+pow(2,64);
  return valUint;
}
//-------------------------------------------------

float ConvertfloatFromReg(uint32_t ValUint32){
   float valuereturn;
union 
  {
    uint32_t x;
  ...
  u.x = ValUint32;
  float tem = u.f;    // 100.34
  valuereturn = roundf(tem*100)/100; // lam tron 2 so thap phan;
return valuereturn;
}

//----------------------------------------------

int16_t ConvertUint2Int16(uint16_t valUint){
  uint16_t valueInt= valUint-pow(2,16);
  return valueInt;
}
int32_t ConvertUint2Int32(int32_t valUint){
  uint32_t valueInt= valUint-pow(2,32);
  return valueInt;
}
int64_t ConvertUint2Int64(int64_t valUint){
  uint64_t valueInt= valUint-pow(2,64);
  return valueInt;
}
...
uint16_t ModbusMaster::readHoldingRegisterUInt16(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 1);
    uint16_t readvalInt16;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt16= ModbusMaster::getResponseBuffer(0);
     }
	 return readvalInt16;
  }

//--------Uint -----------
 uint32_t ModbusMaster::readHoldingRegisterUInt32(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 2);
    uint32_t readvalUInt32;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalUInt32= ModbusMaster::getResponseBuffer(1)<< 16 | ModbusMaster::getResponseBuffer(0);
     }
	 return readvalUInt32;
  }
uint64_t ModbusMaster::readHoldingRegisterUInt64(uint16_t Address){
   result = ModbusMaster::readHoldingRegisters(Address, 4);
   uint64_t readvalUInt64;
    if (result == ModbusMaster::ku8MBSuccess){
      readvalUInt64= ModbusMaster::getResponseBuffer(3)<< 48 | ModbusMaster::getResponseBuffer(2)<<32|ModbusMaster::getResponseBuffer(1)<< 16 | ModbusMaster::getResponseBuffer(0);
      }
	  return readvalUInt64;
  }
  
 float ModbusMaster::readHoldingRegisterFloat32(uint16_t Address){ 
    result = ModbusMaster::readHoldingRegisters(Address, 2);
    float readvalFloat32;
    if (result == ModbusMaster::ku8MBSuccess){
      uint32_t ValUint32 = ModbusMaster::getResponseBuffer(1) << 16 | ModbusMaster::getResponseBuffer(0); 
		readvalFloat32 = ConvertfloatFromReg(ValUint32);
	}
	return readvalFloat32;
 }
  
//----------------------------------------------signed Int--------------------------------------------------
// uint16_t Address in register,            int16_t readvalInt16   Value readed from slave and save in in this 
 uint16_t ModbusMaster::readHoldingRegisterInt16(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 1);
    int16_t readvalInt16;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt16= ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
        }
	 return readvalInt16;
  }
   uint32_t ModbusMaster::readHoldingRegisterInt32(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 2);
    int32_t readvalInt32;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt32= ConvertUint2Int16(ModbusMaster::getResponseBuffer(1))<< 16 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
     }
	 return readvalInt32;
  }
  uint64_t ModbusMaster::readHoldingRegisterInt64(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 4);
    int64_t readvalInt64;
      if (result == ModbusMaster::ku8MBSuccess){
      readvalInt64= ConvertUint2Int16(ModbusMaster::getResponseBuffer(3))<< 48 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(2))<<32|ConvertUint2Int16(ModbusMaster::getResponseBuffer(1))<< 16 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
     }
     return readvalInt64;
  }

//---------READ DATE TIME 16 Bitx4

uint8_t ModbusMaster::readDatetimeHRegister(uint16_t Address){
    result = ModbusMaster::readHoldingRegisters(Address, 4);
    uint16_t tempdaymonth,tempMinHour;
	uint8_t buff[2];

    if (result == ModbusMaster::ku8MBSuccess){
    Year = ModbusMaster::getResponseBuffer(0)+2000; //DONE
	tempdaymonth = ModbusMaster::getResponseBuffer(1);
	tempMinHour  = ModbusMaster::getResponseBuffer(2);
	miliSecond   = ModbusMaster::getResponseBuffer(3);//DONE
	
	Convert16to8bit(tempdaymonth, buff);
	Day = buff[0];//Done
	Month= buff[1];//Done
	
	Convert16to8bit(tempMinHour, buff);
	Minute = buff[0];//DONE
	Hours= buff[1];// DONE
	Second= (uint8_t)miliSecond/1000;
	
     }
	 return result;
  }

uint8_t ModbusMaster::readDatetimeIRegister(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 4);
    uint16_t tempdaymonth,tempMinHour;
	uint8_t buff[2];
...
uint16_t ModbusMaster::readInputRegisterUInt16(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 1);
    uint16_t readvalInt16;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt16= ModbusMaster::getResponseBuffer(0);
     }
 ...
 uint32_t ModbusMaster::readInputRegisterUInt32(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 2);
    uint32_t readvalUInt32;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalUInt32= ModbusMaster::getResponseBuffer(1)<< 16 | ModbusMaster::getResponseBuffer(0);
     }
...
uint64_t ModbusMaster::readInputRegisterUInt64(uint16_t Address){
   result = ModbusMaster::readInputRegisters(Address, 4);
   uint64_t readvalUInt64;
    if (result == ModbusMaster::ku8MBSuccess){
   readvalUInt64= ModbusMaster::getResponseBuffer(3)<< 48 | ModbusMaster::getResponseBuffer(2)<<32|ModbusMaster::getResponseBuffer(1)<< 16 | ModbusMaster::getResponseBuffer(0);
 ...
 float ModbusMaster::readInputRegisterFloat32(uint16_t Address){ 
    result = ModbusMaster::readInputRegisters(Address, 2);
    float readvalFloat32;
    if (result == ModbusMaster::ku8MBSuccess){
      uint32_t ValUint32 = ModbusMaster::getResponseBuffer(1) << 16 | ModbusMaster::getResponseBuffer(0); 
...
//----------------------------------------------signed Int--------------------------------------------------
// uint16_t Address in register,            int16_t readvalInt16   Value readed from slave and save in in this 
 uint16_t ModbusMaster::readInputRegisterInt16(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 1);
    int16_t readvalInt16;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt16= ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
   ...
   uint32_t ModbusMaster::readInputRegisterInt32(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 2);
    int32_t readvalInt32;
    if (result == ModbusMaster::ku8MBSuccess){
     readvalInt32= ConvertUint2Int16(ModbusMaster::getResponseBuffer(1))<< 16 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
  ...
  uint64_t ModbusMaster::readInputRegisterInt64(uint16_t Address){
    result = ModbusMaster::readInputRegisters(Address, 4);
    int64_t readvalInt64;
    if (result == ModbusMaster::ku8MBSuccess){
    readvalInt64= ConvertUint2Int16(ModbusMaster::getResponseBuffer(3))<< 48 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(2))<<32|ConvertUint2Int16(ModbusMaster::getResponseBuffer(1))<< 16 | ConvertUint2Int16(ModbusMaster::getResponseBuffer(0));
...
//
uint8_t ModbusMaster::writeSingleRegisterInt16(uint16_t u16WriteAddress,int16_t i16WriteValue)
{
  uint16_t u16WriteValue=ConvertInt2Uint16(i16WriteValue);   
...
uint8_t ModbusMaster::writeMultipleRegistersU32(uint16_t u16WriteAddress,
  uint32_t u32Writevalue)
{
  Convert32_16bit(u32Writevalue, bufferVal);
  
  ModbusMaster::setTransmitBuffer(0, bufferVal[0]);
  ModbusMaster::setTransmitBuffer(1, bufferVal[1]);
    
  return writeMultipleRegisters(u16WriteAddress,2);
}




uint8_t ModbusMaster::writeMultipleRegistersFloat32(uint16_t u16WriteAddress,
  float FloatWritevalue)
{
  ConvertFloatto2x16bit(FloatWritevalue, bufferVal);
...
uint8_t ModbusMaster::writeMultipleRegistersU64(uint16_t u16WriteAddress,
  uint64_t u64Writevalue)
{
  Convert64_16bit(u64Writevalue, bufferVal);
  
  ModbusMaster::setTransmitBuffer(0, bufferVal[0]);
  ModbusMaster::setTransmitBuffer(1, bufferVal[1]);
  ModbusMaster::setTransmitBuffer(2, bufferVal[2]);
  ModbusMaster::setTransmitBuffer(3, bufferVal[3]);  
   
  return writeMultipleRegisters(u16WriteAddress,4);
}

uint8_t ModbusMaster::writeMultipleRegistersU64_1(uint16_t u16WriteAddress, uint16_t u64Writevalue[4] )
{
  ModbusMaster::setTransmitBuffer(0, u64Writevalue[0]);
  ModbusMaster::setTransmitBuffer(1, u64Writevalue[1]);
  ModbusMaster::setTransmitBuffer(2, u64Writevalue[2]);
  ModbusMaster::setTransmitBuffer(3, u64Writevalue[3]);  
   
  return writeMultipleRegisters(u16WriteAddress,4);
} 

uint8_t ModbusMaster::writeDatetimeHR(uint16_t u16WriteAddress,uint8_t day,uint8_t month,uint16_t year,uint8_t hour,uint8_t minute,uint8_t second){
	ModbusMaster::setTransmitBuffer(0, year-2000);
	ModbusMaster::setTransmitBuffer(1, month<<8|day);
	ModbusMaster::setTransmitBuffer(2, hour<<8|minute);
	uint16_t milisecond= second*1000;
	ModbusMaster::setTransmitBuffer(3, milisecond);
	return  writeMultipleRegisters(u16WriteAddress,4);
}


uint8_t ModbusMaster::writeMultipleRegistersU64_2(uint16_t u16WriteAddress, uint32_t u64Writevalue_L,uint32_t u64Writevalue_H)
{
  result;
  result= ModbusMaster::writeMultipleRegistersU32(u16WriteAddress,u64Writevalue_L); 
  result= ModbusMaster::writeMultipleRegistersU32(u16WriteAddress+2,u64Writevalue_H);
  
   return result;
}


uint8_t ModbusMaster::writeMultipleRegistersInt32(uint16_t u16WriteAddress,int32_t i32Writevalue){
    uint32_t u32Writevalue= ConvertInt2Uint32(i32Writevalue); 
        
    return writeMultipleRegistersU32(u16WriteAddress, u32Writevalue);
} 

uint8_t ModbusMaster::writeMultipleRegistersInt64(uint16_t u16WriteAddress,int64_t i64Writevalue){
    uint64_t u64Writevalue= ConvertInt2Uint64(i64Writevalue); 
    return writeMultipleRegistersU64(u16WriteAddress, u64Writevalue);
} 

uint8_t ModbusMaster::writeMultipleRegistersInt64_1(uint16_t u16WriteAddress,int16_t i64Writevalue[4]){
    uint16_t u64Writevalue[4];
	u64Writevalue[0]= ConvertInt2Uint16(i64Writevalue[0]); 
	u64Writevalue[1]= ConvertInt2Uint16(i64Writevalue[1]); 
	u64Writevalue[2]= ConvertInt2Uint16(i64Writevalue[2]); 
	u64Writevalue[3]= ConvertInt2Uint16(i64Writevalue[3]); 
...
// Methode 2, write int64 bit with 2 32bit int
uint8_t ModbusMaster::writeMultipleRegistersInt64_2(uint16_t u16WriteAddress,int32_t i64Writevalue_L,int32_t i64Writevalue_H){
    uint32_t u64Writevalue_L,u64Writevalue_H;
	u64Writevalue_L= ConvertInt2Uint16(i64Writevalue_L); 
	u64Writevalue_H= ConvertInt2Uint16(i64Writevalue_H); 
	result;
